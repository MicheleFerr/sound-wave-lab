// src/app/(account)/account/ordini/[id]/page.tsx
...
export const metadata: Metadata = {
  ...
  description: 'Visualizza i dettagli del tuo ordine',
}
...
export default async function OrderDetailPage({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Progress Tracker */}
      {!['cancelled', 'refunded', 'pending'].includes(typedOrder.status) && (
        <Card>
                  ...
                  <div key={step} className="flex-1 relative">
                    <div className="flex flex-col items-center">
                      <div
                        ...
                        } ${isCurrent ? 'ring-2 ring-brand-teal ring-offset-2' : ''}`}
                      >
                        {index + 1}
                      </div>
                      <span className={`text-xs mt-2 ${isCompleted ? 'text-foreground' : 'text-muted-foreground'}`}>
                        {stepLabel}
                      </span>
                    </div>
                    {index < statusSteps.length - 1 && (
                      <div
                        className={`absolute top-4 left-1/2 w-full h-0.5 ${
                          index < currentStepIndex ? 'bg-brand-teal' : 'bg-zinc-200 dark:bg-zinc-700'
                        }`}
                      />
                    )}
                  </div>
